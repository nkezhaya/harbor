defmodule Harbor.Web.UserLive.SettingsTest do
  ...
  describe "Settings page" do
    ...
    test "redirects if user is not logged in", %{conn: conn} do
      assert {:error, redirect} = live(conn, "/users/settings")
  ...
  describe "update email form" do
    ...
    test "updates the user email", %{conn: conn, user: user} do
      ...
      {:ok, lv, _html} = live(conn, "/users/settings")
  ...
  describe "confirm email" do
    ...
    test "updates the user email once", %{conn: conn, user: user, token: token, email: email} do
      {:error, redirect} = live(conn, "/users/settings/confirm-email/#{token}")
    ...
    test "does not update email with invalid token", %{conn: conn, user: user} do
      {:error, redirect} = live(conn, "/users/settings/confirm-email/oops")
      assert {:live_redirect, %{to: path, flash: flash}} = redirect
      assert path == "/users/settings"
      assert %{"error" => message} = flash
      assert message == "Email change link is invalid or it has expired."
